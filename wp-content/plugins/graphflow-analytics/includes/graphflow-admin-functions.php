...
function graphflow_admin_form_handler() {
	...
	if ( isset( $_REQUEST['settings-updated'] ) ) {
		...
		if ( $auth == 401 ) {
			...
			add_action( 'admin_notices', 'graphflow_admin_notices_auth_upgrade' );
			return;
		} else {
...
function graphflow_admin_notices_auth_upgrade() {
	echo '<div id="message " class="error notice notice-success is-dismissible below-h2"><p><strong>' .__( 'Your Graphflow trial has expired! Access to Graphflow has been suspended. Please <a href="https://app.graphflow.com/dashboard/account/" target="_blank">upgrade your account</a>.', 'wp-graphflow' ) . '</strong></p></div>';
}
