    ...
    private function send_products_batch($products, $is_products_install_batch = true, $is_retry = false){
    	...
    	if (is_wp_error($resp) || $resp['response']['code'] != 200){
    	    ...
    	    if(isset($response_json['error'])){
    	        update_option('wcis_site_notification', $response_json['error']);
    	    } else {
			...
			if (is_wp_error($resp) || $resp['response']['code'] != 200){			
				...
				if (array_key_exists('server_endpoint', $response_json)){
				    if ($server_endpoing != $response_json['server_endpoint']){
				        update_option('isp_server_endpoint', $response_json['server_endpoint']);
				    }
				}
			}
	...
	function show_admin_message(){			
		if (is_admin()){
			...
			if (get_option('wcic_site_alert')){
			    ...
			    echo '<div class="error" style="background-color:#fef7f1;"><p>';
			    printf(__('<b>InstantSearch+ for WooCommerce</b>: %1$s <a href="%2$s" style="float:right"><b>Hide</b></a>', 'WCISPlugin'),
        			    $msg,
        			    add_query_arg('instantsearchplus', 'remove_site_notification'));
			    echo "</p></div>";
			}
